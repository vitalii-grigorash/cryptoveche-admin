...
const AddNewVoteTypeSamePositions = (props) => {
    ...
    const positionValue = Validation();
    const [materials, setMaterials] = useState([]);
    const [errorMessage, setErrorMessage] = useState('');
    const [isEditMode, setEditMode] = useState(false);
    const [rows, setRows] = useState([]);

    function changePositionValue(e) {
        positionValue.setValue(e.target.value);
    }
    ...
    useEffect(() => {
        if (questionForEdit.id) {
            ...
            setEditMode(true);
            positionValue.setValue(questionForEdit.rules.pick_le);
        } else {
            addEmptyRow();
        ...
        if (isQuestionValid) {
            const question = {
                ...
                rules: {
                    ...
                    pick_lt: -1,
                    pick_gt: -1,
                    pick_le: Number(positionValue.value) < 1 ? 1 : Number(positionValue.value),
                    pick_ge: -1
                }
            }
            if (isEditMode) {
        ...
        <div className="add-new-vote-type-same-positions__container active">
            <div className="add-new-vote-type-same-positions">
                <div className="add-new-vote-type-same-positions__title">
                    <h3 className="add-new-vote-type-same-positions__title-number-question">{!isEditMode && `${constants.ADD_NEW_VOTE.QUESTION} #${questionsList.length + 1}`}</h3>
                ...
                <div className="add-new-vote-type-same-positions__number-positions-block">
                    ...
                    />
                    <input
                        className="add-new-vote-type-same-positions__numbers-position"
                        type="number"
                        placeholder='1'
                        min={1}
                        step={1}
                ...
                <div className="add-new-vote-type-same-positions__types-variants-answer">
                    <div className="add-new-vote-type-same-positions__types-variants-answer-input-text">
                        ...
                        {rows.sort(function (a, b) {
                            ...
                            <div key={row.id} className="add-new-vote-type-same-positions__type-input-block">
                                <input
                                ...
                                <div className="add-new-vote-type-same-positions__type-input-icons">
                                    <img className="add-new-vote-type-same-positions__type-input-gray-plus" onClick={addEmptyRow} src={iconPlus} alt={constants.GENERAL.ALT_ICON} />
                                    <img className="add-new-vote-type-same-positions__type-input-gray-basket" onClick={() => deleteRow(row.id)} src={iconBasket} alt={constants.GENERAL.ALT_ICON} />
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="add-new-vote-type-same-positions__add-button-block">
                        <button className="add-new-vote-type-same-positions__add-btn" onClick={addNewQuestion}>{isEditMode ? constants.ADD_NEW_VOTE.SAVE_CHANGES : constants.ADD_NEW_VOTE.ADD_NEW_VOTE_ADD_QUESTION_BTN}</button>
                    </div>
                </div>
            </div>
        </div>
